...
export const CustomerTable = ({
  ...
  return <>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-slate-200">
              <th className="text-left py-3 px-4 font-medium text-slate-600 cursor-pointer hover:bg-slate-100" onClick={() => handleSort('name')}>
                <div className="flex items-center gap-2">
                  Customer
                  {getSortIcon('name')}
                </div>
              </th>
              <th className="text-left py-3 px-4 font-medium text-slate-600 cursor-pointer hover:bg-slate-100" onClick={() => handleSort('email')}>
                <div className="flex items-center gap-2">
                  Contact
                  {getSortIcon('email')}
                </div>
              </th>
              <th className="text-left py-3 px-4 font-medium text-slate-600 cursor-pointer hover:bg-slate-100" onClick={() => handleSort('totalPurchases')}>
                <div className="flex items-center gap-2">
                  Total Purchases
              ...
              <th className="text-left py-3 px-4 font-medium text-slate-600 cursor-pointer hover:bg-slate-100" onClick={() => handleSort('lastPurchaseDate')}>
                <div className="flex items-center gap-2">
                  Last Purchase
                  {getSortIcon('lastPurchaseDate')}
                </div>
              </th>
              <th className="text-left py-3 px-4 font-medium text-slate-600 cursor-pointer hover:bg-slate-100" onClick={() => handleSort('status')}>
                <div className="flex items-center gap-2">
                  Status
                  {getSortIcon('status')}
                </div>
          ...
          <tbody>
            ...
            return <tr key={customer.id} className="border-b border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="py-4 px-4">
                    {editingDiscount === customer.id ? <div className="flex items-center gap-2">
                        <Input type="number" value={tempDiscount} onChange={e => setTempDiscount(Number(e.target.value))} className="w-20 h-8" min="0" max="100" />
                        <Button size="sm" onClick={() => handleDiscountSave(customer.id)} className="h-8 px-2">
                          ✓
                        </Button>
                        <Button size="sm" variant="ghost" onClick={handleDiscountCancel} className="h-8 px-2">
                          ✕
                        </Button>
                      </div> : <div className="flex items-center gap-1 cursor-pointer hover:bg-slate-100 rounded px-2 py-1" onClick={() => handleDiscountEdit(customer)}>
                  ...
                  <td className="py-4 px-4">
                   ...
                   <td className="py-4 px-4">
                     <div className="flex items-center gap-2">
                       <Badge variant={customer.status === 'active' ? 'default' : 'secondary'} className={customer.status === 'active' ? 'bg-emerald-100 text-emerald-800 hover:bg-emerald-100' : 'bg-slate-100 text-slate-600 hover:bg-slate-100'}>
                         {customer.status.charAt(0).toUpperCase() + customer.status.slice(1)}
                       </Badge>
                       <Button size="sm" variant="ghost" onClick={() => setStatusDialogCustomer(customer)} className="h-6 w-6 p-0">
                  ...
                  <td className="py-4 px-4">
                    <div className="flex items-center justify-end gap-2">
                      {onView && <Button size="sm" variant="outline" onClick={() => onView(customer)} className="h-8 px-2">
                          <Eye className="w-4 h-4" />
                        </Button>}
                      {onCommunicate && <Button size="sm" variant="outline" onClick={() => onCommunicate(customer)} className="h-8 px-2">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          {onCreateInvoice && <DropdownMenuItem onClick={() => onCreateInvoice(customer)}>
      ...
      <CustomerStatusDialog customer={statusDialogCustomer} open={statusDialogCustomer !== null} onClose={() => setStatusDialogCustomer(null)} onStatusUpdated={() => {
      onRefresh?.();
